...
namespace EasyDapper.Extension.MySql
{
    ...
    internal static class ResolveExpression
    {
        ...
        public static string ResolveLimit(int? topNum)
        {
            if (!topNum.HasValue || topNum <= 0)
                return "";
            return $" LIMIT 0, {topNum.Value} ";
        }
        ...
        public static string ResolveSelect(PropertyInfo[] propertyInfos, LambdaExpression selector)
        {
            ...
            {
                var select = new SelectExpression(propertyInfos, selector, null, ProviderOption);
        ...
        public static string ResolveCount(PropertyInfo[] propertyInfos, LambdaExpression selector)
        {
            ...
            switch (nodeType)
            {
                case ExpressionType.MemberAccess:
                    var columnName = ((MemberExpression)selector.Body).Member.GetColumnAttributeName();
                    selectSql = $" SELECT IFNULL(Count({ProviderOption.CombineFieldName(columnName)}),0)  ";
                    break;
                case ExpressionType.MemberInit:
                    throw new DapperExtensionException("不支持该表达式类型");
            }
        ...
        public static string ResolveAvg(PropertyInfo[] propertyInfos, LambdaExpression selector)
        {
            ...
            switch (nodeType)
            {
                case ExpressionType.MemberAccess:
                    var columnName = ((MemberExpression)selector.Body).Member.GetColumnAttributeName();
                    selectSql = $" SELECT IFNULL(AVG({ProviderOption.CombineFieldName(columnName)}),0)  ";
                    break;
                case ExpressionType.MemberInit:
                    throw new DapperExtensionException("不支持该表达式类型");
            }
        ...
        public static string ResolveMin(PropertyInfo[] propertyInfos, LambdaExpression selector)
        {
            ...
            switch (nodeType)
            {
                case ExpressionType.MemberAccess:
                    var columnName = ((MemberExpression)selector.Body).Member.GetColumnAttributeName();
                    selectSql = $" SELECT IFNULL(Min({ProviderOption.CombineFieldName(columnName)}),0)  ";
                    break;
                case ExpressionType.MemberInit:
                    throw new DapperExtensionException("不支持该表达式类型");
            }
        ...
        public static string ResolveMax(PropertyInfo[] propertyInfos, LambdaExpression selector)
        {
            ...
            switch (nodeType)
            {
                case ExpressionType.MemberAccess:
                    var columnName = ((MemberExpression)selector.Body).Member.GetColumnAttributeName();
                    selectSql = $" SELECT IFNULL(Max({ProviderOption.CombineFieldName(columnName)}),0)  ";
                    break;
                case ExpressionType.MemberInit:
                    throw new DapperExtensionException("不支持该表达式类型");
            }
