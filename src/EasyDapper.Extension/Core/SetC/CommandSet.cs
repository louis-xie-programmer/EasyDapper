...
namespace EasyDapper.Extension.Core.SetC
{
    ...
    public class CommandSet<T> : Command<T>, ICommandSet<T>
    {
        ...
        public void BatchInsert(IEnumerable<T> entities, int timeout = 240)
        {
            SqlProvider.ExcuteBulkCopy(DbCon, entities, timeout);
        }

        public void BatchInsert(DataTable dt, int timeout = 240)
        {
            SqlProvider.ExcuteBulkCopy(DbCon, dt, timeout);
        }
    }
}
